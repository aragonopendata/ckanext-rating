...
from ckan.plugins.toolkit import ValidationError, get_action
...
@pytest.mark.parametrize('rating', [0, -1, MAX_RATING + 1, 1.5, '3', 'a', None, ''])
def test_cannot_create_rating_with_invalid_rating(user, package, rating):
    ...
    context = {'user': user['name']}
    data_dict = {'package_id': package['id'], 'rating': rating}
    create_rating = get_action('rtng_create_rating')
    with pytest.raises(ValidationError):
        create_rating(context, data_dict)
